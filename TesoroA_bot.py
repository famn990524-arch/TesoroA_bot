...
# ======================
# FUNZIONI THREAD (VARIAZIONI) - NON MODIFICARE
# ======================
...
def reset_fotos_per_iguser(iguser: str):
    """Resetta tutte le foto per un determinato iguser"""
    global fotos_global_state
...
def aggiungere_foto_per_iguser(iguser: str, foto_path: str):
    """Aggiunge una foto al pool di un iguser specifico"""
    global fotos_global_state
...
def ottenere_foto_disponibili_per_iguser(iguser: str, quantita: int) -> List[int]:
    """Ottiene foto casuali non ancora usate per un iguser"""
    if iguser not in fotos_global_state:
        return []
...
def marcare_foto_come_usate_per_iguser(iguser: str, foto_ids: List[int]):
    """Marca le foto come usate e le rimuove dai disponibili"""
    if iguser not in fotos_global_state:
        return
...
def get_stato_fotos_per_iguser(iguser: str) -> tuple:
    """Restituisce (usate, disponibili, total) per un iguser"""
    if iguser not in fotos_global_state:
        return 0, 0, 0
...
# ======================
# COMANDI ADMIN - FOTO PER IGUSER
# ======================

async def caricare_foto_iguser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando /caricarefoto_iguser NOMBRE - Prepara per caricare foto per un iguser specifico"""
    user = update.effective_user
    user_id = user.id
    ...
    # Estraiamo il nome dell'iguser dal comando
    text = update.message.text
    match = re.match(r'^/caricarefoto_(\w+)$', text.lower())
    if not match:
        await update.message.reply_text(
            "❌ **Formato errato!**\n\n"
            "Usa: <code>/caricarefoto_iguser</code>\n"
            "es. <code>/caricarefoto_mariorossi</code>\n\n"
...
async def ricevere_foto_iguser(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def done_fotos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Aggiungere tutte le foto al pool dell'iguser
...
async def stato_foto_iguser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando /stato_foto_iguser - Mostra stato pool foto per un iguser"""
    user = update.effective_user
    if user.id != ADMIN_USER_ID:
    ...
    text = update.message.text
    match = re.match(r'^/stato_foto_(\w+)$', text.lower())
    if not match:
        await update.message.reply_text("❌ Usa: <code>/stato_foto_iguser</code>", parse_mode="HTML")
        return
    ...
    await update.message.reply_text(
        ...
        f"📌 Usa /caricarefoto_{iguser} per aggiungere altre foto.",
        parse_mode="HTML"
    )

async def reset_foto_iguser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando /reset_foto_iguser - Resetta tutte le foto per un iguser"""
    user = update.effective_user
    if user.id != ADMIN_USER_ID:
        await update.message.reply_text("❌ Solo @famn25 può usare questo comando.")
        return
    ...
    text = update.message.text
    match = re.match(r'^/reset_foto_(\w+)$', text.lower())
    if not match:
        await update.message.reply_text("❌ Usa: <code>/reset_foto_iguser</code>", parse_mode="HTML")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• <code>/Nfoto_iguser</code> - Ricevi N foto usa e getta\n\n"
        f"📌 <b>Esempi:</b>\n"
        f"• <code>/12threads</code> → 12 variazioni\n"
        f"• <code>/3foto_mariorossi</code> → 3 foto usa e getta\n\n"
...
async def foto_comando(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Gestisce i comandi /Nfoto_iguser"""
    user = update.effective_user
    user_id = user.id
    username = user.username or user.first_name
    text = update.message.text
    
    match = re.match(r'^/(\d+)foto_(\w+)$', text.lower())
    if not match:
        return
    
    quantita = int(match.group(1))
    iguser = match.group(2)
    
    if quantita < 1:
        await update.message.reply_text("❌ Usa /1foto_iguser o più")
        return
    ...
    if iguser not in fotos_global_state or fotos_global_state[iguser]["total"] == 0:
        await update.message.reply_text(
            ...
            f"L'admin deve caricare foto con /caricarefoto_{iguser}"
        )
        return
    
    usate, disponibili, total = get_stato_fotos_per_iguser(iguser)
    
    # Avisar al admin si quedan pocas fotos (solo admin, no el usuario)
    if disponibili <= THRESHOLD_FOTOS and disponibili > 0:
        await notificare_admin(
            context,
            f"⚠️ <b>ATTENZIONE - POCHE FOTO PER @{iguser}!</b>\n"
            f"📸 Foto disponibili: {disponibili}\n"
            f"📌 Usa /caricarefoto_{iguser} per caricare altre foto.",
    ...
    await notificare_admin(context, f"📸 @{username} ha richiesto {quantita} foto per @{iguser}", is_admin_action=False)
    
    foto_ids = ottenere_foto_disponibili_per_iguser(iguser, quantita)
    
    # Messaggio di inizio
    await update.message.reply_text(f"📸 Invio di {len(foto_ids)} foto per @{iguser}...", parse_mode="HTML")
    ...
    # Solo mensaje de confirmación, sin información de cuántas quedan
    await update.message.reply_text(
        f"✅ <b>Foto inviate!</b>\n\n"
...
async def aiuto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    messaggio_base = (
        ...
        f"• <code>/Nfoto_iguser</code> - Ricevi N foto usa e getta (es. /3foto_mariorossi)\n"
    ...
    if user_id == ADMIN_USER_ID:
        messaggio_admin = (
            ...
            f"│  • <code>/caricarefoto_iguser</code> - Carica foto per un utente Instagram\n"
            f"│  • <code>/stato_foto_iguser</code> - Mostra stato pool foto\n"
            f"│  • <code>/reset_foto_iguser</code> - Resetta pool foto\n"
...
def main():
    # Inizializzare cartelle e database
    os.makedirs(DATA_FOLDER, exist_ok=True)
    os.makedirs(PHOTOS_FOLDER, exist_ok=True)
    ...
    # Comandi admin - Foto
    application.add_handler(CommandHandler("caricarefoto_", caricare_foto_iguser, block=False))
    application.add_handler(CommandHandler("stato_foto_", stato_foto_iguser, block=False))
    application.add_handler(CommandHandler("reset_foto_", reset_foto_iguser, block=False))
    ...
    application.add_handler(MessageHandler(filters.Regex(r'^/\d+foto_\w+$'), foto_comando))
    
    # Handler per ricevere file
    application.add_handler(MessageHandler(filters.Document.ALL, ricevere_file_frase))
    application.add_handler(MessageHandler(filters.PHOTO | filters.Document.IMAGE, ricevere_foto_iguser))
    ...
    print("=" * 60)
    print("📌 COMANDI AMIN - FRASI:")
    print("  • /caricare_frase")
    print("  • /verificare_frase")
    print("=" * 60)
    print("📌 COMANDI ADMIN - FOTO USA E GETTA:")
    print("  • /caricarefoto_iguser")
    print("  • /stato_foto_iguser")
    print("  • /reset_foto_iguser")
    print("  • /done")
    print("=" * 60)
    print("📌 COMANDI UTENTI:")
    print("  • /Nthreads -> variazioni casuali")
    print("  • /Nfoto_iguser -> foto usa e getta")
